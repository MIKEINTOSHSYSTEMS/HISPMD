...
function CustomExpression($value, $data, $field, $ptype, $table="")
{
	...
	if(!$table)
		$table = $strTableName;
				if($table=="sgb_suggestions" && $field=="suggested_by")
	{
		$urs = DB::Select($_SESSION["ltable"],array($_SESSION["lkey"]=>$data["suggested_by"]));
$udata = $urs->fetchAssoc();
if($udata){
	$value = GetCustomLabel("suggested_by").": <b>".$udata[GetDisplayNameField()]."<span class='admin_m'>".GetCustomLabel("admin")."</span></b>";
}
else{
	$value = GetCustomLabel("suggested_by").": <b>".$data["name"]."</b>";
}
;
		return $value;
	}
				if($table=="sgb_suggestions" && $field=="status")
	{
		$rs = DB::Select("sgb_status",array("statusname" =>$data["status"] ));
$data = $rs->fetchAssoc();
$value = "<span class='colored_status' data-status='".$data["statusname"]."' style='background:".$data["statuscolor"]."'>".$value."</span>";
;
		return $value;
	}
				if($table=="sgb_suggestions" && $field=="created_date")
	{
		$value =  " (".runner_date_format("j M, 'y",strtotime($data["created_date"])).")";;
		return $value;
	}
				if($table=="sgb_suggestions" && $field=="upvoted_date")
	{
		if($data["upvoted_date"] === "0000-00-00 00:00:00"){
	$value = "Upvoted:none";
}
else{
	$value = "Upvoted:".getHowAgo($data["upvoted_date"]);
}
;
		return $value;
	}
				if($table=="sgb_suggestions" && $field=="vote")
	{
		$is_user_voited_rs = DB::Select("sgb_votes",array("ip" => $_SERVER["REMOTE_ADDR"],"s_id" => $data["id"]));
$is_user_voited = $is_user_voited_rs->fetchAssoc();
$value = "<span class='votes'>".$value."</span><span class='vlabel'>".GetCustomLabel("votes")."</span>";
$link = GetTableLink("sgb_comments","list")."?masterkey1=".$data["id"]."&mastertable=sgb_suggestions";
$class = "";
if($is_user_voited){
		$class.="votedup";
}
$voite_condition = "<span class='sLabel unvoite'><i class='fa fa-check'></i>".GetCustomLabel("voted_up")."</span>";

if(postvalue("mastertable") === "sgb_suggestions"){
	$class.= " mastertable";
	$value = $value."<i class='fa fa-arrow-circle-o-up'></i>";
	$link = "#";
$voite_condition.= "<span class='sLabel voteb'>".GetCustomLabel("upvote")."</span>";
	
}
$value = "<a class='".$class."' href='".$link."'>".$value.$voite_condition."</a>";

;
		return $value;
	}
				if($table=="sgb_suggestions" && $field=="categoryid")
	{
		$rs = DB::Select("sgb_categories",array("id" =>$data["categoryid"] ));
$datacat = $rs->fetchAssoc();
$link = GetTableLink("sgb_suggestions","list")."?category=".$datacat["url"];
$value = "<span class='colored_category' style='background:".$datacat["category_color"]."'><a href='".$link."'>".$value."</a></span>";
;
		return $value;
	}
				if($table=="sgb_suggestions" && $field=="title")
	{
		if(postvalue("mastertable")!=="sgb_suggestions") {
	$link = GetTableLink("sgb_comments","list")."?masterkey1=".$data["id"]."&mastertable=sgb_suggestions";
	$value = "<a href='".$link."'>".$data["title"]."</a>";
};
		return $value;
	}
				if($table=="sgb_suggestions" && $field=="comments")
	{
		$commentCount = DBLookup("select count(*) from ".addTableWrappers("sgb_comments")." where ".addFieldWrappers("s_id")."=".$data["id"]);
$link = GetTableLink("sgb_comments","list")."?masterkey1=".$data["id"]."&mastertable=sgb_suggestions";
$value = "<a class='comments_link' href='".$link."'>".GetCustomLabel("comments").":".$commentCount."</a>";;
		return $value;
	}
				if($table=="sgb_suggestions" && $field=="pinned")
	{
		$value="";
if(!intval($data["pinned"]) && Security::getUserGroup()==="admin"){
	$value="<span data-action='pin' data-id='".$data["id"]."' class='pina pinthis fa fa-bookmark'>".GetCustomLabel("pinthis")."</span>";
}
if(intval($data["pinned"])){
	$value="<span data-id='".$data["id"]."' class='pinned fa fa-bookmark'>".GetCustomLabel("pinned")."</span>";
	if(Security::getUserGroup()==="admin")
		$value.="<span data-action='unpin' data-id='".$data["id"]."' class='pina unpin fa fa-close'>".GetCustomLabel("unpin")."</span>";
}
...
;
		return $value;
				...
				if($table=="sgb_categories" && $field=="viewlive")
	{
		$value = "<a href='".GetTableLink("sgb_suggestions","list")."?category=".$data["url"]."'>View live</a>";;
		return $value;
	}
				if($table=="sgb_comments" && $field=="user_id")
	{
		
$urs = DB::Select($_SESSION["ltable"],array($_SESSION["lkey"]=>$data["user_id"]));
$udata = $urs->fetchAssoc();
$you = "";
if($udata){
	if($data["user_id"] == $_SESSION["uid"] && !intval($data["merged_info"])) $you = "(".GetCustomLabel("you").")";
	$value = "<b>".$udata[GetDisplayNameField()].$you."<span class='admin_m'>Admin</span></b>";
}
else{
	if($data["user_id"] == $_COOKIE["sgb_gsid"] && !intval($data["merged_info"]) ) $you = "(".GetCustomLabel("you").")";
	$value = "<b>".$data["name"].$you."</b>";
}
if(intval($data["merged_info"])) 
	$value.="<span class='merged_info'>Merged</span>";
...
;
		return $value;
				...
				if($table=="sgb_comments" && $field=="comment")
	{
		if($data["status"] === "awaiting_approval"){
$value = "<span class='aapproval'>Awaiting approval</span>".$value;
}
...
;
		return $value;
				...
				if($table=="sgb_comments" && $field=="created_date")
	{
		$value =  "".runner_date_format("j M, 'y",strtotime($data["created_date"]))."";;
		return $value;
	}
				if($table=="sgb_comments" && $field=="status")
	{
		$statuses_labels = array(
	"awaiting_moderation"=>"Awaiting approval",
	"approved"=>"Approved",
	"deleted"=>"Deleted",
	"spam"=>"Spam"
);
$value = "<span class='colored_status' status='".$data["status"]."'>".$statuses_labels[$data["status"]]."</span>";
;
		return $value;
	}
				if($table=="sgb_comments" && $field=="admin_buttons")
	{
		$value = "<div class='comment_moderate_panel'>";
$edit_link = GetTableLink("sgb_comments","edit","editid1=".$data["id"]."&masterkey1=".$_SESSION["smaster"]["id"]."&&mastertable=sgb_suggestions"); 
$value.="<a href='".$edit_link."' class='btn btn-default editaction'>Edit</a>";
if(Security::getUserGroup() === "admin"){
$buttonsa = array(
	array("status" => "awaiting_moderation","label"=>GetCustomLabel("awaiting_approval"),"fa" =>null),
	array("status" => "approved","label"=>GetCustomLabel("approved"),"fa" =>"check"),
	array("status" => "deleted","label"=>GetCustomLabel("deleted"),"fa" =>"trash"),
	array("status" => "spam","label"=>GetCustomLabel("spam"),"fa" =>"exclamation")
);
$awaiting_moderation = "";


$value.="<div class='status_buttons' data-comment-id='".$data["id"]."'>";
foreach($buttonsa as $butt){
	$fa = "";
	if(!is_null($butt["fa"]))
		$fa = "<i class='fa fa-".$butt["fa"]."'></i>";
	$active = "";
	if($butt["status"] === $data["status"])
		$active = "active";
	$value.="<span class='btn btn-default ".$active."' data-status='".$butt["status"]."'>".$fa.$butt["label"]."</span>";
...
$value.="</div>";;
		return $value;
				...
				if($table=="sgb_admin_suggestions" && $field=="suggested_by")
	{
		$urs = DB::Select($_SESSION["ltable"],array($_SESSION["lkey"]=>$data["suggested_by"]));
$udata = $urs->fetchAssoc();
$value = "1";
if($udata){
	$value = "<a href='".GetTableLink("contributor","dashboard")."?aname=".rawurlencode($data["name"])."&autor=".rawurlencode($data["email"])."'>".$udata[GetDisplayNameField()]."</a><br>".$udata[GetEmailField()];
}
else{
	$value = "<a href='".GetTableLink("contributor","dashboard")."?aname=".rawurlencode($data["name"])."&autor=".rawurlencode($data["email"])."'>".$data["name"]."</a><br>".$data["email"];
}
;
		return $value;
	}
				if($table=="sgb_admin_suggestions" && $field=="status")
	{
		$rs = DB::Select("sgb_status",array("statusname" =>$data["status"] ));
$data = $rs->fetchAssoc();
$value = "<span class='colored_status' style='background:".$data["statuscolor"]."'>".$value."</span>";
;
		return $value;
	}
				if($table=="sgb_admin_suggestions" && $field=="created_date")
	{
		$value = "<b>".runner_date_format("j M, 'y",strtotime($data["created_date"]))."</b>";;
		return $value;
	}
				if($table=="sgb_admin_suggestions" && $field=="upvoted_date")
	{
		if($data["upvoted_date"] === "0000-00-00 00:00:00"){
	$value = "Upvoted:none";
}
...
;
		return $value;
				...
				if($table=="sgb_admin_suggestions" && $field=="vote")
	{
		$is_user_voited_rs = DB::Select("sgb_votes",array("user_id" => $_SESSION["uid"],"s_id" => $data["id"]));
$is_user_voited = $is_user_voited_rs->fetchAssoc();
$value = "<span class='votes'>".$value."</span><span class='vlabel'>".GetCustomLabel("votes")."</span>";
$link = GetTableLink("sgb_comments","list")."?masterkey1=".$data["id"]."&mastertable=sgb_suggestions";
$class = "";
...
$voite_condition = "<span class='sLabel unvoite'><i class='fa fa-check'></i>Voted up</span>";

if(postvalue("mastertable") === "sgb_suggestions"){
	$class.= " mastertable";
	$value = $value."<i class='fa fa-arrow-circle-o-up'></i>";
	$link = "#";
$voite_condition.= "<span class='sLabel voteb'>Upvote</span>";
...
;
		return $value;
				...
				if($table=="sgb_admin_suggestions" && $field=="categoryid")
	{
		$rs = DB::Select("sgb_categories",array("id" =>$data["categoryid"] ));
$datacat = $rs->fetchAssoc();
$link = GetTableLink("sgb_suggestions","list")."?category=".$datacat["url"];
...
;
		return $value;
				...
				if($table=="sgb_admin_suggestions" && $field=="title")
	{
		if(postvalue("mastertable")!=="sgb_suggestions") {
	$link = GetTableLink("sgb_comments","list")."?masterkey1=".$data["id"]."&mastertable=sgb_suggestions";
	$value = "<a href='".$link."'>".$data["title"]."</a>";
		...
		return $value;
				...
				if($table=="sgb_admin_suggestions" && $field=="comments")
	{
		$commentCount = DBLookup("select count(*) from ".addTableWrappers("sgb_comments")." where ".addFieldWrappers("s_id")."=".$data["id"]);
$link = GetTableLink("sgb_comments","list")."?masterkey1=".$data["id"]."&mastertable=sgb_suggestions";
$value = "<a class='comments_link' href='".$link."'>".GetCustomLabel("comments").":".$commentCount."</a>";;
		return $value;
				...
				if($table=="sgb_admin_comments" && $field=="id")
	{
		$value = "<a class='fa fa-external-link' href='".GetTableLink("sgb_comments","list")."?masterkey1=".$data["s_id"]."&mastertable=sgb_suggestions&commentid=".$data["id"]."'></a>";;
		return $value;
	}
				if($table=="sgb_admin_comments" && $field=="user_id")
	{
		$urs = DB::Select($_SESSION["ltable"],array($_SESSION["lkey"]=>$data["user_id"]));
$udata = $urs->fetchAssoc();
$value = "1";
...
;
		return $value;
				...
				if($table=="sgb_admin_comments" && $field=="created_date")
	{
		$value =  "<b>".runner_date_format("j M, 'y",strtotime($data["created_date"]))."</b>";;
		return $value;
	}
				if($table=="sgb_admin_comments" && $field=="s_id")
	{
		$link = GetTableLink("sgb_comments","list")."?masterkey1=".$data["s_id"]."&mastertable=sgb_suggestions";
$value = "<a href='".$link."'>".$value."</a>";;
		return $value;
	}
				if($table=="sgb_admin_comments" && $field=="editlink")
	{
		$link = GetTableLink("sgb_comments","edit")."?editid1=".$data["id"];
$value = "<a href='".$link."' class='fa fa-pencil'></a>";;
		return $value;
	}
				if($table=="sgb_users_suggestions" && $field=="title")
	{
		if(postvalue("mastertable")!=="sgb_suggestions") {
	$link = GetTableLink("sgb_comments","list")."?masterkey1=".$data["id"]."&mastertable=sgb_suggestions";
	$value = "<a href='".$link."'>".$data["title"]."</a>";
		...
		return $value;
				...
				if($table=="sgb_users_suggestions" && $field=="blockip")
	{
		$is_blocked_rs = DB::Select("sgb_blockedip",array("ip" => $data["ip"]));
$is_blocked = $is_blocked_rs->fetchAssoc();
if(!$is_blocked)
	$value = "<span data-action='block' class='btn btn-default' data-ip='".$data["ip"]."'>Block ip</span>";
else
	$value = "<span data-action='unblock' class='btn btn-default' data-ip='".$data["ip"]."'>Unblock ip</span>";;
		return $value;
	}
				if($table=="sgb_users_comments" && $field=="id")
	{
		$value = "<a class='fa fa-external-link' href='".GetTableLink("sgb_comments","list")."?masterkey1=".$data["s_id"]."&mastertable=sgb_suggestions&commentid=".$data["id"]."'></a>";;
		return $value;
	}
				if($table=="sgb_users_comments" && $field=="user_id")
	{
		$urs = DB::Select($_SESSION["ltable"],array($_SESSION["lkey"]=>$data["user_id"]));
$udata = $urs->fetchAssoc();
$you = "";
if($udata){
	if($data["user_id"] == $_SESSION["uid"]) $you = "(".GetCustomLabel("you").")";
	$value = "<b>".$udata[GetDisplayNameField()].$you."<span class='admin_m'>Admin</span></b>";
}
else{
	if($data["user_id"] == $_COOKIE["sgb_gsid"]) $you = "(".GetCustomLabel("you").")";
	$value = "<b>".$data["name"].$you."</b>";
}
...
;
		return $value;
				...
				if($table=="sgb_users_comments" && $field=="created_date")
	{
		$value =  "".runner_date_format("j M, 'y",strtotime($data["created_date"]))."";;
		return $value;
	}
				if($table=="sgb_users_comments" && $field=="blockip")
	{
		$is_blocked_rs = DB::Select("sgb_blockedip",array("ip" => $data["ip"]));
$is_blocked = $is_blocked_rs->fetchAssoc();
if(!$is_blocked)
	$value = "<span class='btn btn-default' data-action='block' data-ip='".$data["ip"]."'>Block ip</span>";
else
	$value = "<span class='btn btn-default' data-action='unblock' data-ip='".$data["ip"]."'>Unblock ip</span>";;
		return $value;
	}
				if($table=="sgb_merge" && $field=="id")
	{
		$value = "#".$value;;
		return $value;
	}
				if($table=="sgb_merge" && $field=="suggested_by")
	{
		$urs = DB::Select($_SESSION["ltable"],array($_SESSION["lkey"]=>$data["suggested_by"]));
$udata = $urs->fetchAssoc();
if($udata){
	$value = GetCustomLabel("suggested_by").": <b>".$udata[GetDisplayNameField()]."<span class='admin_m'>Admin</span></b>";
}
else{
	$value = GetCustomLabel("suggested_by").": <b>".$data["name"]."</b>";
}
;
		return $value;
				...
				if($table=="sgb_merge" && $field=="status")
	{
		$rs = DB::Select("sgb_status",array("statusname" =>$data["status"] ));
$data = $rs->fetchAssoc();
$value = "<span class='colored_status' data-status='".$data["statusname"]."' style='background:".$data["statuscolor"]."'>".$value."</span>";
...
;
		return $value;
				...
				if($table=="sgb_split" && $field=="title")
	{
		if(postvalue("mastertable")!=="sgb_suggestions") {
	$link = GetTableLink("sgb_comments","list")."?masterkey1=".$data["id"]."&mastertable=sgb_suggestions";
	$value = "<a href='".$link."'>".$data["title"]."</a>";
		...
		return $value;
				...
				if($table=="sgb_split" && $field=="status")
	{
		$rs = DB::Select("sgb_status",array("statusname" =>$data["status"] ));
$data = $rs->fetchAssoc();
$value = "<span class='colored_status' data-status='".$data["statusname"]."' style='background:".$data["statuscolor"]."'>".$value."</span>";

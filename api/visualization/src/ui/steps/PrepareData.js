  ...
  this.userData_ = new chartEditor.ui.userData.Widget([
    {
      ...
      type: 'upload',
      caption: 'CSV String',
      icon: './icons/pngs/csv-string.png'
    },
    {
      id: 'file-csv',
      type: 'upload',
      caption: 'CSV File',
      icon: './icons/pngs/csv-file.png'
    },
    {
      id: 'string-json',
      type: 'upload',
      caption: 'JSON String',
      icon: './icons/pngs/json-string.png'
    },
    {
      id: 'file-json',
      type: 'upload',
    ...
    },
    {
      ...
      icon: './icons/pngs/google-spreadsheet.png'
    }
  ]);
  ...
  if (dataType !== 'spreadsheets' && (typeOf === 'object' || typeOf === 'array')) {
    ...
    switch (dataType) {
      ...
      case 'csv':
        ...
        // Ensure first row is used as labels
        csvSettings['useFirstRowAsFields'] = true;
        ...
        if (arrResult.length > 1) {
          // Extract the first row, which contains the field names (labels)
          var fields = goog.array.splice(arrResult, 0, 1)[0];

          // Log to verify fields
          console.log('Extracted fields:', fields);

          for (var i = 0; i < arrResult.length; i++) {
            var item = arrResult[i];  // Access each row
            var res = {};
            for (var j = 0; j < item.length; j++) {
              var name = fields[j] || ('Field ' + j);  // Default to 'Field N' if no label
              res[name] = item[j];  // Map the data to the corresponding label
            }
            result.push(res);  // Push the labeled data into the result
          }
        } else {
          result = arrResult;  // If there's only one row, directly use it
        }
        break;

...
$url = "https://viz.hispmd.merqconsultancy.org/api/card/87/query/json";
...
if ($response === false) {
    ...
    foreach ($apiData as $item) {
       ...
       // $adminUnit = $item["Unit → Unit Name"];
        ...
        if ($year > 0) {  // Ensure the year is positive
            ...
            $aggregatedData[$year][$indicatorName] += (float)$cleanedValue;
        }
    }
    ...
    $output = [
        "name" => "Proportion of Births and Deaths Notified",
        "data" => [],
        "chartType" => "funnel",
        "defaults" => [
            [
            ...
            [
                ...
                "value" => "Proportion of Births and Deaths Notified"
            ]
        ]
    ];
    ...
    foreach ($aggregatedData as $year => $facilityData) {
        ...
        foreach ($facilityData as $indicatorName => $value) {
            $output['data'][] = [

...
foreach ($data['data'] as $item) {
    ...
    $transformed_entry = [
        ...
        "Year" => isset($item['Year']) ? DateTime::createFromFormat('Y', $item['Year'])->format('Y') : "", // Date as "YYYY-01-01"
        //"Year" => isset($item['Year']) ? (int)$item['Year'] : "", // Integer
...
// Sort the transformed data by the "Year" field in ascending order
usort($transformed, function($a, $b) {
    return $a['Year'] <=> $b['Year']; // Compare years in ascending order
});

...
foreach ($data['data'] as $entry) {
    ...
    if (!isset($groupedIndicators[$indicatorGroup][$indicatorName])) {
        $groupedIndicators[$indicatorGroup][$indicatorName] = [
            'years' => [],
            'description' => [],
            'abbreviation' => strtolower($entry['Data Source'] ?? 'Unknown Source')
        ];
    }
...
foreach ($groupedIndicators as $group => $indicators) {
    foreach ($indicators as $name => $data) {
        ...
        // Add input for further selections
        $furtherInputs = [
            'dataSource' => $entry['Data Source'] ?? null,
            'assessment' => $entry['Assessment'] ?? null,
            'facilityType' => $entry['Facility Type'] ?? null,
            'administrationUnit' => $entry['Administration Unit'] ?? null
        ];

        foreach ($furtherInputs as $key => $value) {
            if (!empty($value)) {
                $inputItem = $defaults->addChild('input');
                $inputItem->addAttribute('name', $key);
                $inputItem->addAttribute('value', htmlspecialchars($value));
                $inputItem->addAttribute('type', 'string'); // Assuming the type for these inputs is string
            }
        }
    }
}
